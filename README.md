...
# Note: Always run as a module (-m) to avoid relative import errors
python -m src.run_eval --config configs/gsm8k_smoke.yaml

# Alternatively, use the Makefile if available:
# make smoke
...
> [!NOTE]
> Code-task evaluation executes candidate code in a subprocess; run these in a sandboxed environment.
...
python scripts/aggregate_results.py --latest_group
python scripts/plot_pareto.py --latest_group
```
...
## Troubleshooting
- **`ModuleNotFoundError: No module named 'src'`**: Make sure you run python using the module syntax (e.g., `python -m src.run_eval`). Do not run scripts directly like `python src/run_eval.py` as this breaks relative imports.
